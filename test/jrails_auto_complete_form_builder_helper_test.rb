...
class JrailsAutoCompleteFormBuilderHelperTest < Test::Unit::TestCase
  ...
  def test_compare_to_macro_in_normal_form
    ...
    fields_for(@person) do |f|
      ...
      assert_equal 'person', f.object_name
    end
  ...
  def test_compare_to_macro_in_normal_form_with_symbol
    ...
    fields_for(:some_class) do |f|
      _erbout.concat f.text_field_with_auto_complete(:name)
      assert_equal 'some_class', f.object_name
    end
  ...
  def test_compare_macro_to_fields_for_in_nested_form
    ...
    puts
    puts standard_auto_complete_html
    puts
    puts _erbout
    puts
    puts _erbout.gsub(/group\[person_attributes\]\[\]/, 'person').gsub(/person_[0-9]+_name/, 'person_name').gsub(/paramName:'person\[name\]'/, '')
    puts
